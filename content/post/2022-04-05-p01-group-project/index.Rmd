...
```{r}
ChurnLog = glm(Exited ~ Tenure,  data=train, family=binomial)
summary(ChurnLog)
```
#### Tenure = -0.00811
#### Train a logistic regression model using Age as the independent variable. What is the coefficient of Age?
```{r}
ChurnLog = glm(Exited ~ Age,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using Geography as the independent variable. What is the coefficient of Geography?
```{r}
ChurnLog = glm(Exited ~ Geography,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using Gender as the independent variable. What is the coefficient of Gender?
```{r}
ChurnLog = glm(Exited ~ Gender,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using NumOfProducts as the independent variable. What is the coefficient of NumOfProducts?
```{r}
ChurnLog = glm(Exited ~ NumOfProducts,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using HasCrCard as the independent variable. What is the coefficient of HasCrCard?
```{r}
ChurnLog = glm(Exited ~ HasCrCard,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using IsActiveMember as the independent variable. What is the coefficient of IsActiveMember?
```{r}
ChurnLog = glm(Exited ~ IsActiveMember,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using EstimatedSalary as the independent variable. What is the coefficient of EstimatedSalary?
```{r}
ChurnLog = glm(Exited ~ EstimatedSalary,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using Balance as the independent variable. What is the coefficient of Balance?
```{r}
ChurnLog = glm(Exited ~ Balance,  data=train, family=binomial)
summary(ChurnLog)
```
#### Train a logistic regression model using CreditScore as the independent variable. What is the coefficient of CreditScore?
```{r}
ChurnLog = glm(Exited ~ CreditScore,  data=train, family=binomial)
summary(ChurnLog)
```
...
#### Using logistic regression model, obtain predictions on the test set. Then, using a probability threshold of 0.5, create a confusion matrix for the test set. What is the accuracy of logistic regression model?
```{r}
predictTrain = predict(ChurnLog, newdata = test, type="response")
table(test$Exited, predictTrain > 0.5)
z = table(test$Exited, predictTrain > .1)
sum(diag(z))/sum(z)
```
#### Accuracy = 0.7963333
